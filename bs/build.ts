...
class Header {
  ...
  public toString(isToplevel: boolean = true) {
    ...
    const genIncludes = (): [ string[], string[] ] => {
      ...
      let extIncs: string[] = [];
      let intIncs: string[] = [];
      for (const inc of systemDeps)
        extIncs.push(inc.toString(false).trim());

      if (libDeps.length) {
        extIncs.push('\n');
        extIncs.push(`// ${'embedded libraries'.padStart(80 - 7)} {{{`);
        for (const inc of libDeps)
          extIncs.push(inc.toString(false));
        extIncs.push(`// ${'embedded libraries'.padStart(80 - 7)} }}}`);
      }

      for (const inc of projectDeps)
        intIncs.push(inc.toString(false));

      return [ extIncs, intIncs ];
    };

    let s: string = '';
    if (isToplevel && this.copyright.length)
      s += this.copyright + '\n\n';
    ...
    if (this.body.length) {
      s += this.body.replace(/^namespace CCUTL_NAMESPACE {$/m, '')
               .replace(/^} \/\/ namespace CCUTL_NAMESPACE$/m, '') +
           '\n';
    }
    if (this.type === HeaderType.project && isToplevel) {
    ...
    if (!isToplevel && this.type !== HeaderType.system)
      return this.vimfold(s);
    else
      return s;
  };

  protected constructor({type, name, relPath, copyright, included, body}: {
    type: HeaderType,
    name: string,
    relPath: string,
    copyright: string,
  ...
  private guard(s: string, isTopLevel: boolean): string {
    ...
    const cleanName = this.name.toLowerCase()
  ...
  constructor(relPath: string) {
    if (PROJECT_HEADERS.find(x => x.relPath === relPath))
      throw new Error(`project header ${
          relPath} already exists in registry. please construct using createOrGetProjectHeader`);
    // read the file
    let lines = (() => {
      let p = path.join(Project.srcDir, relPath);
      if (!fs.existsSync(p))
        p = path.join(Project.rootDir, relPath);
    ...
    const name = (() => {
      ...
      return relPath.replace(/^ctl/m, 'CCUTL')
          .replace(/\//g, '_')
          .replace(/\.h$/m, '')
          .toUpperCase()
    ...
    const includes = (() => {
      ...
      for (const ln of lines) {
        ...
        if (!m)
          removedIncLines.push(ln);
        else {
          // do not add detail config
          if (m[1].match(/^src\/ctl\/detail\/config\.h$/m))
            continue;
          ...
          try {
            ...
            if ((<Error>e).message.match(/^found more than one/m))
              throw e;
            else
              incs.push(createOrGetSystemHeader(m[1]));
          }
        }
      }
...
class LibHeader extends Header {
  constructor(relPath: string) {
    if (LIB_HEADERS.find(x => x.relPath === relPath))
      throw new Error(`lib header ${
          relPath} already exists in registry. please construct using createOrGetLibHeader`);
...
class SystemHeader extends Header {
  constructor(relPath: string) {
    if (SYSTEM_HEADERS.find(x => x.relPath === relPath))
      throw new Error(`system header ${
          relPath} already exists in registry. please construct using createOrGetSystemHeader`);
    super({
...
function expandDependencies(): void {
  function recurse(dept: Header, dep: Header) {
    ...
    for (const dep_ of Array.from(deps))
      recurse(dept, dep_);
  }
...
function sortDependencies(): void {
  for (const dep of Array.from(DEPENDENCIES.keys())) {
    ...
    for (const h of sorted) {
      switch (h.type) {
      case (HeaderType.project): projectSorted.push(h); break;
      case (HeaderType.lib): libSorted.push(h); break;
      case (HeaderType.system): systemSorted.push(h); break;
      };
    }

    systemSorted.sort(
        (a, b) => a.relPath < b.relPath   ? -1
                  : a.relPath > b.relPath ? 1
                                          : 0);

    DEPENDENCIES.set(
        dep, new Set([...systemSorted, ...libSorted, ...projectSorted ]));
  }
}
...
(() => {
  ...
  for (const h of PROJECT_HEADERS) {
    // skip if detail
    if (h.relPath.match(/^ctl\/detail\//m))
      continue;
