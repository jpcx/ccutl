...
- a module named `std` is created internally (simple system header inclusion in the global module fragment caused many errors)
- this `std` module includes ALL C++ API headers and is re-exported from each module in the project that imports it
...
```
```cmake
set(CCUTL_MODULES ON) # this is required for modules
add_subdirectory([ccutl download dir])
target_link_libraries([your target] ccutl)
...
ccutl.reduce | performs a reduction operation on the provided range or arguments
